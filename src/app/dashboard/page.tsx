...
export default function Dashboard() {
  ...
  const uniqueIndustries = useMemo(() => {
    ...
    const allIndustries = leadsData
      .map((lead: Lead) => lead.industry)
      .filter(Boolean);
  ...
  const uniqueStatuses = useMemo(() => {
    ...
    const allStatuses = leadsData
      .map((lead: Lead) => lead.email_status)
      .filter(Boolean);
  ...
  const uniqueCountries = useMemo(() => {
    ...
    const allCountries = leadsData
      .map((lead: Lead) => lead.country)
      .filter(Boolean);
  ...
  const uniqueLeadStatuses = useMemo(() => {
    ...
    const allLeadStatuses = leadsData
      .map((lead: Lead) => lead.lead_status)
      .filter(Boolean);
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1 min-w-[200px]">
          <a
            ...
            className="text-blue-400 hover:underline text-sm flex items-center gap-1 truncate"
          >
          ...
          <Badge
            variant={
              row.original.email_status === "Verified" ? "default" : "secondary"
            }
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1 text-sm">
          ...
          <span>
            {row.getValue("employee_count")?.toLocaleString() || "N/A"}
          </span>
        </div>
      ),
    },
        ...
        <div className="flex gap-2">
          ...
          {row.original.person_linkedin_url && (
            ...
            >
              ...
              {/* Use ExternalLink for simplicity */}
            </a>
          )}
        </div>
      ),
            ...
            <div className="border-t border-gray-800 bg-gray-900/30 p-4 flex justify-between items-center flex-wrap gap-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-400 px-3">
                  ...
                  of <span className="text-white font-medium">{pageCount}</span>
                </span>
                <Button
                  variant="outline"
                  size="sm"
